...
class WhatsApp
{
    ...
    {
        $Order = Order::where('id',$id)->with(['Client','Address'=>['Region','Country'],'Payment','Products.Item'=>['Size','Color']])->firstorfail();
        ...
        foreach($Order->Products as $Product){
            $Item = $Product->Items->where('color_id',$Product->pivot->color_id)->where('size_id',$Product->pivot->size_id)->first();
            $message .= '%0a *Item :* ' .       strip_tags($Product->title_en);
            if($Product->Size)
                $message .= '%0a *Size :* ' .      $Item->title_en;
            if($Product->Color)
            $message .= '%0a *Color :* ' .      $Item->title_en;
            $message .= '%0a *Price :* ' .      number_format( Country()->currancy_value *  $Product->pivot->price, Country()->decimals , '.', '') . ' ' . Country()->currancy_code;
            $message .= '%0a *Quantity :* ' .   $Product->pivot->quantity;
            $message .= '%0a *Total :* ' .      number_format( Country()->currancy_value *  $Product->pivot->total, Country()->decimals , '.', '') . ' ' . Country()->currancy_code;
            $message .= '%0a';
            $message .= '%0a';
        }

        ...
        @foreach ($Models as $Setting)
            ...
            @elseif (str_contains($Setting['key'], '_services') || (str_contains($Setting['key'], 'ar') || str_contains($Setting['key'], 'en') || $Setting['type'] == 'meta') && !str_contains($Setting['key'], 'title_'))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang('trans.'.$Setting['key'])</label>
                        <textarea style="min-height: 300px" class="form-control {{ (str_contains($Setting['key'], 'about_ar') || str_contains($Setting['key'], 'about_en') || str_contains($Setting['key'], 'inOurWebsite_ar') || str_contains($Setting['key'], 'inOurWebsite_en') || str_contains($Setting['key'], 'address_ar') || str_contains($Setting['key'], 'address_en'))  ? '' : 'editor' }} mceNoEditor" id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}" required placeholder="@lang('trans.' . $Setting['key'])">{!! $Setting['value'] !!}</textarea>
                    </div>
                </div>
